test_that("afttest linApprox=TRUE runs correctly", {
  ...
  result = afttest(object = Surv(Time, status) ~ z1 + z2, data = simdata,
                   ...
                   linApprox = TRUE)
  expect_equal(result$p_value, 0.02, tolerance=1e-1)
  ...
  result = afttest(object = Surv(Time, status) ~ z1 + z2, data = simdata,
                   ...
                   linApprox = TRUE)
  expect_equal(result$p_value, 0.04, tolerance=1e-1)
  expect_equal(result$p_std_value, 0.03, tolerance=1e-1)
  ...
  result = afttest(object = Surv(Time, status) ~ z1 + z2, data = simdata,
                   ...
                   linApprox = TRUE)
  expect_equal(result$p_value, 0.02, tolerance=1e-1)
  expect_equal(result$p_std_value, 0.01, tolerance=1e-1)
})
